...
namespace APIs
{
    public static class DatabaseAPI
    {
        ...
        public static void Remove(string path, string json, Action callback, Action<AggregateException> fallback)
        {
            var customReference = GetReferenceFromPath("lobby/users");
            //customReference.RemoveValueAsync("as")
            //customReference.RemoveValueAsync();
            //Query query = customReference.Child("users").OrderByChild("uid").EqualTo(json);
            ...
            customReference.GetValueAsync().ContinueWith(task =>
            {
                if (task.IsCompleted)
                {
                    ...
                    foreach (DataSnapshot snapshotChild in snapshot.Children)
                    {
                        ...
                        //Debug.Log(child);
                        var oo = JsonUtility.FromJson<LobbyUser>(child);
                        //Debug.Log(snapshotChild.Children);
                        

                        if (oo.uid == json)
                        {
                            snapshotChild.Reference.RemoveValueAsync();
                            //Debug.Log(oo.uid);
                        }
                        //snapshotChild.
        ...
        private static void RemoveHandleValueChanged(object sender, ValueChangedEventArgs args)
        {
        ...
        public static void PushValue(string path, string val, Action callback, Action<AggregateException> fallback)
        {
            var customReference = GetReferenceFromPath(path);

             customReference.Push().SetValueAsync(val).ContinueWith(task =>
            {
                if (task.IsCanceled)
                {
                ...
                //Debug.Log("Value pushed successfully");
                callback();
            });
        }
        ...
        public static void GetAllDatas(string path)
        {
            var customReference = GetReferenceFromPath(path);
            ...
            customReference.GetValueAsync().ContinueWith(task =>
            {
                ...
                if (task.IsCompleted)
                {
                    ...
                    //foreach (DataSnapshot snapshotChild in snapshot.Children)
                    //{ 
                    //    Debug.Log(snapshotChild.GetRawJsonValue());
        ...
        public static void GetAllDatas<T>(string path, T obj, Action<List<T>> callback)
        {
            var customReference = GetReferenceFromPath(path);
        ...
        public static void AddLobby()
        {
            var id = Auth.Instance.currentUser.userUID;
            string json = "{" + '"' + "uid"  + '"' + ":" + '"' + id + '"' + "}";
            //Debug.Log(json);    
            //PushValue("lobby/users", json, null, null);
            PushJSON("lobby/users", json, null, null);
            ListenForChildAdded("lobby/users", LobbyListener, null);


        }

        public static void LobbyListener(ChildChangedEventArgs args)
        {
